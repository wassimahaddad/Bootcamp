...
const toString2 = floatNumbers.map((el) => {
...
const names = movies.map((el) => el.name);
...
const titleAsObjects = movies.map((el) => {
  return { name: el.name, genre: el.genres };
});
