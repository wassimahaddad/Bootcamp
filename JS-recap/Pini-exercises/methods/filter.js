...
// With the filter method return from the function only the numbers that are greater than the number you gave as an argument.
const numbers = [1, 100, 67, -5, 6784, 356, 200];
const greaterThanN = (arr, n) => arr.filter((number) => number > n);
console.log(greaterThanN(numbers, 5));

// 3.2 – Greater than and less than.
//
// Use the same array of numbers as in the previous exericse
//
// Create a function that takes 3 arguments, an array, and 2 numbers.With the filter method
//
// return from the function only the numbers that are greater than and less than the numbers you supplied to the function.
// E.g. if your first number is 10 and second number is 300 you should only
// return the numbers 100, 67, 200
const betweenNs = (arr, n1, n2) =>
  arr.filter((number) => number > n1 && number < n2);
console.log(betweenNs(numbers, 50, 500));

// Use the movie data for the following exercises:
//
// 3.3 – filter rating
//
// Create a function that takes two arguments, an array and a rating number
// Using the filter method return from the function only the movies that the rating average is more than the rating number provided in the function.
...
const filterRatings = (arr, rating) =>
  arr.filter((movie) => movie.rating.average > rating);
console.log(filterRatings(movies, 7));
